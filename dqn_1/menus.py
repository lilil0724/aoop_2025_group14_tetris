...
def lan_menu(screen, font):
    ...
    while True:
        ...
        # Draw Player Count Label
        # Move label above the buttons to avoid overlap
                ...
                for btn in buttons:
                    if btn.is_clicked(event):
                        if btn.action_code == "BACK":
                            ...
                            while waiting:
                                # 檢查連線狀態
                                current_players = len(net_mgr.clients) + 1 # Host + Clients
                                ...
                                for e in pg.event.get():
                                    ...
                                    if btn_start.is_clicked(e):
                                        net_mgr.start_game() # Signal start
                                        return "LAN", net_mgr
                            ...
                            while waiting:
                                # 1. Check Connection & Game Start
                                if net_mgr.connected:
                                    if net_mgr.game_started:
                                        return "LAN", net_mgr
                                    
                                    # Connected but waiting for host to start
                                    screen.fill(config.background_color)
                                    ...
                                    for e in pg.event.get():
                                        ...
                                        if e.type == pg.KEYDOWN and e.key == pg.K_ESCAPE:
                                            net_mgr.close(); waiting = False; net_mgr = network_utils.NetworkManager()
