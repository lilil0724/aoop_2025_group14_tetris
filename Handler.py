...
def isDefeat(shot, piece):
    ...
    # (修改) 檢查 is_fixed，並且檢查所有 cell
    if getattr(piece, "is_fixed", False) and any(y < 0 for y, _ in getCellsAbsolutePosition(piece)):
        return True
        
    # (新增) 3) 檢查是否有任何固定方塊被推出頂部
    # (這主要由 insertGarbage 觸發)
    if any(shot.status[0][x] == 2 for x in range(config.columns)):
         # 稍微寬鬆一點，給 y=0 一格緩衝，如果 y=0 且在動的方塊也在 y=0，isDefeat 會抓
         # 嚴格一點：只要 status[0] 有 2 就判輸
         pass # 讓 1) 去抓
